...
TFT_Display::TFT_Display(uint8_t cs, uint8_t cd, uint8_t wr, uint8_t rd, uint8_t reset) : Adafruit_TFTLCD(cs, cd, wr, rd, reset) {
...
void TFT_Display::begin(uint16_t id) {
  uint8_t i = 0;
...
void TFT_Display::setAddrWindow(int16_t x1, int16_t y1, int16_t x2, int16_t y2) {
  CS_ACTIVE;
  if(driver == ID_932X) {
...
void TFT_Display::flood(uint16_t color, uint32_t len) {
  uint16_t blocks;
...
void TFT_Display::drawFastHLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t x2;
...
void TFT_Display::drawFastVLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t y2;
...
uint16_t TFT_Display::color565(uint8_t r, uint8_t g, uint8_t b) {
  return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
}

void TFT_Display::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h, 
  uint16_t fillcolor) {
  int16_t  x2, y2;
...
void TFT_Display::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void TFT_Display::pushColors(uint16_t *data, uint8_t len, boolean first) {
  uint16_t color;
  uint8_t  hi, lo;
  CS_ACTIVE;
...
void TFT_Display::setRotation(uint8_t x) {
...
uint16_t TFT_Display::readPixel(int16_t x, int16_t y) {
...
uint16_t TFT_Display::readID(void) {
...
uint32_t TFT_Display::readReg(uint8_t r) {
  uint32_t id;
  uint8_t x;
...
#ifndef write8
void // For I/O macros that were left undefined, declare function
// versions that reference the inline macros just once:

#ifndef write8
void TFT_Display::write8(uint8_t value) {
  write8inline(value);
}
#endif

#ifdef read8isFunctionalized
uint8_t TFT_Display::read8fn(void) {
  uint8_t result;
  read8inline(result);
  return result;
}
...
#ifndef setWriteDir
void TFT_Display::setWriteDir(void) {
  setWriteDirInline();
}
#endif

#ifndef setReadDir
void TFT_Display::setReadDir(void) {
  setReadDirInline();
}
#endif

#ifndef writeRegister8
void TFT_Display::writeRegister8(uint8_t a, uint8_t d) {
  writeRegister8inline(a, d);
}
#endif

#ifndef writeRegister16
void TFT_Display::writeRegister16(uint16_t a, uint16_t d) {
  writeRegister16inline(a, d);
}
#endif

#ifndef writeRegisterPair
void TFT_Display::writeRegisterPair(uint8_t aH, uint8_t aL, uint16_t d) {
  writeRegisterPairInline(aH, aL, d);
}
#endif::write8(uint8_t value) {
  write8inline(value);
}
#endif
...
void TFT_Display::writeRegister24(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
...
void TFT_Display::writeRegister32(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
...
void TFT_Display::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
{
...
void TFT_Display::WriteCmdData(uint16_t cmd, uint16_t dat) { writecmddata(cmd, dat); }
